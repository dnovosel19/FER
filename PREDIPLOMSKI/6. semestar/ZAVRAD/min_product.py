...
def vertex_edge_logarit(vertex_edge, logaritamske_vrijednosti):     # logaritmiraj svaku tezinu i napravi takav graf
    for brid in vertex_edge:
        logaritamske_vrijednosti.append((brid[0], brid[1], round(math.log(brid[2]), 3)))
...
def PrimMST(graph, vertex_edge, pocetak):   # Primov algoritam na 'logaritamskom' grafu
    closed = set()
    closed.add(pocetak)
    open = []
    while True:
...
if __name__ == '__main__':
    ...
    vert_ed_log = []    # graf sa logaritamskim vrijednostima
    prikaz_graf = []    # ispis rjesenja grafa

    vertex_edge_logarit(vertex_edge, vert_ed_log)
    PrimMST(graph, vert_ed_log, 0)

    prod = 1
    for i in graph:         # izracun produkta
        for j in vertex_edge:
            if (i[0] == j[0]) and (i[1] == j[1]):
                prod *= j[2]
                prikaz_graf.append((i[0], i[1], j[2]))
    print(prikaz_graf)
    print(prod)

    nacrtaj_graf(prikaz_graf)