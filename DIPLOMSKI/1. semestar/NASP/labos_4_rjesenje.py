...
def query_priority_subtree(tree: List[Node], index: int, limit_value: float) -> List[Point]:
    ...
    if node.point.y >= limit_value: # TODO: Replace the condition
...
def query(tree: List[Node], interval: Tuple[float, float], limit: float) -> List[Point]:
    ...
    while node is not None and node is not INF_NODE and \
        ...
        point = node.point
        if interval[0] <= node.point.x <= interval[1] and node.point.y >= limit: # TODO: Replace the condition
            result.append(point)
    ...
    if interval[0] <= node.med:
        result.extend(query_priority_subtree(tree, left_index, limit))

    right_index = 2*index + 2
    if interval[1] >= node.med:
        result.extend(query_priority_subtree(tree, right_index, limit))
    return result